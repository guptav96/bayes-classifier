...
def nbc(t_frac):
    ...
    for attribute in attributes:
        numerator = sampled_train_df.groupby(by = ['decision', attribute]).size()
        denominator = sampled_train_df.groupby(by = 'decision').size()
...
def accuracy(original_labels, predicted_labels):
    ...
    for idx in range(total_num):
        if original_labels[idx] == predicted_labels[idx]:
            count += 1
    return round(float(count)/total_num,2)
